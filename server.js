...
const mailTmRequest = async (
  ...
  if (!response.ok) {
    ...
    if (response.status === 404) {
      errorMessage = errorMessage || "Mail.tm service endpoint not found. The service may be temporarily unavailable.";
      ...
      errorMessage = errorMessage || "Mail.tm service is temporarily unavailable. Please try again later.";
    } else {
      errorMessage = errorMessage || `Mail.tm request failed (${response.status})`;
    }
...
app.post("/api/mailboxes", async (req, res, next) => {
  try {
    ...
    if (error.status === 404) {
      error.message = "Mail.tm service is temporarily unavailable. Please try again in a few moments.";
    } else if (error.message?.includes("page could not be found") || error.message?.includes("NOT_FOUND")) {
      error.message = "Mail.tm service is temporarily unavailable. Please try again in a few moments.";
